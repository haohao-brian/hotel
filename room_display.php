...
if ($email!="" && $password!="") {//這邊先設定不能讓使用者輸入空值
  ...
  // 建立SQL指令字串
    ...
    if ($total_records==null){
      ...
      echo "<h1><font color= #FF7575>登入失敗，請確認帳號密碼是否填寫正確<br></h1>";
      echo '</button><a href="room_login.php"><button>再試一次</a></button></font></div>';
    }else {
  ...
  <div class="slide-right" style="width:225px;height:100px;border:0px white dashed;position: absolute; top: 0px; right:450px;">
    ...
    <form name="login" action="room_update.php" method="post">
      <table align="center" bgcolor="transparent" class="table">
            ...
            <tr>
                 <td colspan="2" ><div align="right"><button><input type="submit" value="更新個資"/></button></td>
                <td colspan="2" ><div align="right"><button><input type="submit" value="訂房資料" onclick='this.form.action="room_status.php"' /></button></td>
            </tr>        
      </table>
    </form>
  </body>
